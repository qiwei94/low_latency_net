...
parser.add_argument('--dir2', '-d2',
                    help="Directory to store outputs,but not clean",
                    required=True)
...
class DCTopo(Topo):
    ...
    def __init__(self, n=3):
        ...
        self.addLink('h0', switch, cls=Link, cls1=TCIntf, cls2=TCIntf, 
                      params1=receiverConfig2, params2=switchConfig)

        # Adding links from the switch to the senders (hosts)
        for i in range(1,n):
          #self.addLink('h%s' % i, switch, **senderConfig2)
          self.addLink('h%s' % i, switch100)

        os.system("sudo ifconfig eth1 up")
        os.system("sudo ifconfig eth2 up")

        #os.system("sudo ifconfig eth4 up")
        os.system("sudo ovs-vsctl add-port s0 eth1")
        os.system("sudo ovs-vsctl add-port s100 eth2")
...
def start_iperf(net):
    ...
    server = h0.popen("iperf -s -w 16m > %s/iperf-server-%s.txt " %(args.dir2,number),shell=True)
    print "start sockperf ..."
    sockperf_server = h0.popen("sockperf server -p 5005 --tcp")
    ...
    for i in range(1,args.n):
      ...
      client = h.popen("iperf -c %s -p 5001 -t 20 > %s/iperf-%d.txt " %(h0.IP(),args.dir,i), 
                      shell=True)
    h1 = net.get("h1")
    sleep(3)
    sockperf_h1_client = h1.popen("sockperf pp -i %s -t 5 -p 5005 --tcp > %s/sockperf-%s.txt" %(h0.IP(),args.dir2,number),shell=True)
...
def dctcp():    
    ...
    net.start()
    ##set controller need to set 
    os.system("sudo ovs-vsctl set Bridge s0 protocol=OpenFlow13")
    os.system("sudo ovs-vsctl set Bridge s100 protocol=OpenFlow13")
    
    os.system("sudo ovs-vsctl set-controller s0 tcp:192.168.50.8:6633")
    os.system("sudo ovs-vsctl set-controller s100 tcp:192.168.50.8:6633")
...
def enableDCTCP():
    ...
    os.system("sudo cat /proc/sys/net/ipv4/tcp_congestion_control")
    os.system("echo kiwi:::::::::::::::::::::::::::::use dctcp")

def disableDCTCP():
    os.system("sudo sysctl -w net.ipv4.tcp_congestion_control=cubic")
    os.system("sudo cat /proc/sys/net/ipv4/tcp_congestion_control")
    os.system("echo kiwi:::::::::::::::::::::::::::::not use dctcp,use cubic")
